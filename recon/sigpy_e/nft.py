...
def nufft_adj1(data, traj, dcf, device = sp.Device(-1), smap = None, batch = 40000, id_channel = False, ishape = None):
    xp = device.xp
        ...
        with device:
            for c in range(num_coils):
                ...
                for seg in range((num_tr-1)//batch+1):
                    ksp_ttc = sp.to_device(ksp_t[c,seg*batch:np.minimum((seg+1)*batch,num_tr),...], device)
                    coord_tt = sp.to_device(coord_t[seg*batch:np.minimum((seg+1)*batch,num_tr),...], device)
                    dcf_tt = sp.to_device(dcf_t[seg*batch:np.minimum((seg+1)*batch,num_tr),...], device)
        ...
        with device:
            ...
            for c in range(num_coils):
                ...
                for seg in range((num_tr-1)//batch+1):
                    ...
                    ksp_t[c,seg*batch:np.minimum((seg+1)*batch,num_tr),...] = sp.to_device(ksp_tt)
...
def nufft_adj(data, traj, dcf, device = sp.Device(-1), smap = None, batch = 40000, id_channel = False, ishape = None):
    xp = device.xp
    N_phase = len(data)
    img = []
    for num_ph in range(N_phase):
